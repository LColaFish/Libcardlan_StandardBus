...
int InitPsam(int PsamNum, int BaudRate)
{
    int psam_num_temp   = 0;
    int  ret ;
    ...
    LOGI("PsamNum: %d\n", PsamNum);
	LOGI("psam_num_temp=0x%2x\n", psam_num_temp);
	if (psam_num_temp == 0x00){
        psam_num_temp = 0x61;
        LOGI("can't get PsamNum , change to Index 5 automatically");
    }
    /* Open */
    {
    ...
    LOGI("psam set baudrate success\n");
    /* Cold Reset */
    {
    ...
    }
    {
		...
		menu_print(recv_data,recv_len[0]);
   
	LOGI("psam cold reset success\n");
		LOGI(" recv_data:%02x %02x %02x %02x %02x %02x \n",recv_data[0],recv_data[1],recv_data[2],recv_data[3],recv_data[4]);
		
		PsamAtr[PsamNum][0] = recv_data[3];
		LOGI("psam[%d] atr len:%d\n",PsamNum,PsamAtr[PsamNum][0]);
		memcpy(&PsamAtr[PsamNum][1],&recv_data[8],recv_data[3]);
...
{
    return GetPsamID_ex(PsamIndex,PsamNum,&PsamKeyIndex);
}

int PsamCmd(int PsamNum,unsigned char *sendcmd,int sendlen,unsigned char *rcvcmd,int *rcvlen)
{
	int ret = 0;
	unsigned char len[4];
	unsigned char buff[256]={0};
	unsigned char getResp[]={0x00,0xc0,0x00,0x00,0x00};
	LOGI("Psam_Cmd_Send before sending");
	ret = Psam_Cmd_Send(PsamNum,sendcmd,sendlen,buff,len);
    LOGI("Psam_Cmd_Send Success");	
	#if 0
            ...
            case 1:
                {
                    ...
                    if(ret != 0 || buff[len[0]-2] != 0x90 || buff[len[0]-1] != 0x00)
                    {
                        ...
                        LOGI("ret = %d , Command Order: ",ret);
                        menu_print(getpsamnum,sizeof(getpsamnum));
                        menu_print(buff,len[0]);
                        Loop  = 0;
                        break;
                    }
...
int  Psam_Cmd_Send(int PsamNum,unsigned        char* send_cmd, unsigned char send_cmd_len, char* recv_cmd, unsigned char *recv_cmd_len)
{
    int psam_num_temp   = 0;
    int  ret                      = 0;
