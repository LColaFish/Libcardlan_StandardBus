...
int InitPsam(unsigned char PsamNum, int BaudRate)
{
    ...
    /* Cold Reset */
    {
		...
		PsamAtr[PsamNum][0] = recv_len;
		memcpy(&PsamAtr[PsamNum][1],recv_data,recv_len);
    }
    return 0;
}
...
int  Psam_Cmd_Send(unsigned char PsamNum,unsigned   char* send_cmd, unsigned char send_cmd_len, char* recv_cmd, unsigned char *recv_cmd_len)
{
    ...
    { 
        ...
        ret = mcu_cmd_read_and_write(psam_num_temp,0x04,&read_data_len,data_len,recv_buff,recv_cmd_len);
        if(ret != 0)
        {
            return -3;
        }
        memcpy(recv_cmd, recv_buff+8, recv_cmd_len[0] + recv_cmd_len[1] + recv_cmd_len[2] + recv_cmd_len[3]);
    }
    return 0;
}
