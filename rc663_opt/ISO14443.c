...
//
//******************************************************************************
unsigned char ISO14443_COS(unsigned short *cpLen, unsigned char *cpAPDU,unsigned char *rAPDU)
{
    ...
    //printf("g_sPcdPara.cFWI=%02x,g_sPcdPara.cCID=%x,g_sPcdPara.cNAD=%x,g_sPcdPara.cMaxINF=%d\n",g_sPcdPara.cFWI,g_sPcdPara.cCID,g_sPcdPara.cNAD,g_sPcdPara.cMaxINF);
	PcdSetTmo(g_sPcdPara.cFWI);
    memset(g_sPcdTrsv.cData,0,512);
	...
	do
	{
		...
		else if ((g_sPcdTrsv.cData[0] & 0xF0) == 0x10)		// 收到I(1)数据   发送R(ACK)应答
		{
			...
			memcpy(&rAPDU[iRecvIndex], &g_sPcdTrsv.cData[cCurIndex], cRecvLenNow);
	...
	//	else if (((g_sPcdTrsv.cData[0] & 0xF2) == 0x02)||((g_sPcdTrsv.cData[0] & 0xF2) == 0x00))		// 收到I(0)数据   结束
...
//		{
	...
	//		}
//		}
//        DBGPRINTK("ISO14443_COS cStatus=%02x\n",cStatus);
    //    print_char(g_sPcdTrsv.iLength, rAPDU);
...
//
//******************************************************************************
unsigned char PiccAuthState_ex(unsigned char cBlk, unsigned char cMode, unsigned char *snr, unsigned char *cKey)
{
	unsigned char cStatus;
	...
	memcpy(&g_sPcdTrsv.cData[2], snr, 4);
	g_sPcdTrsv.iLength = 6;
    ...
    cStatus = ReadRawRC(PHHAL_HW_RC663_REG_STATUS);
    if(cStatus & 0x20 == 0)
    {
        return MI_AUTHERR;
    }
...
{
    return  PiccAuthState_ex(cBlk,cMode,g_sCard.cUID, cKey);
}
//******************************************************************************
...
unsigned char PiccRead(unsigned char cAddr, unsigned char *cpData)
{
	...
	if (cStatus == ERR_SUCCESS)
	{
		...
		{
			memcpy(cpData, g_sPcdTrsv.cData, 0x10);
		}
	}
	return cStatus;
}
...
unsigned char PiccWrite(unsigned char cBlock, unsigned char *cpData)
{
	...
	{
		PcdSetTmo(5);
		memcpy(g_sPcdTrsv.cData, cpData, 0x10);
