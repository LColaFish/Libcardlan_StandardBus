...
int search_1card(	uint8_t bUid[CHIP_ID_LENGTH_MAX])
{
	uint8_t bSak[1];
...
int active_1card(uint8_t bUid[CHIP_ID_LENGTH_MAX])
{
	uint8_t bSak[1];
...
int rc531_test(void)
{
    ...
    ret = init_1dev(device);
    if(ret != 0)
    {
        return -1;
    }
    ret = search_1card(bUid);
    if(ret != 0)
    {
        return -2;
    }
