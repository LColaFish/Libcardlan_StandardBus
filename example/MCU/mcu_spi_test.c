...
static int mcu_spi_test(char psamnum)
{
 ...
 /*发命令*/
	...
	/*选择AID*/
	{
		unsigned char recv_data[256];
		unsigned char recv_len[4];
		unsigned char send_len[] = {18,0,0,0}; 
		//unsigned char send_data[5] = {0x00,0xB0,0x96,0x00,0x06};
		unsigned char send_data[32] = {0x00,0xa4,0x04,0x00,0x0d,0xa0,0x00,0x00,0x00,0x03,0x53,0x49,0x4e,0x4f,0x50,0x45,0x43,0x31};
		ret = PsamCmd(psamnum, send_data, send_len[0], recv_data, recv_len);	
		if(ret != 0)
		{
			printf("[%s %d]  error ret:%d!!\n",__FUNCTION__,__LINE__,ret);
			return ret;
	...
	/*选择文件*/
	{
		...
		//unsigned char send_data[5] = {0x00,0xB0,0x96,0x00,0x06};
		unsigned char send_data[32] = {0x00,0xb0,0x97,0x0,0x0};
		ret = PsamCmd(psamnum, send_data, send_len[0], recv_data, recv_len);	
		if(ret != 0)
		{
			printf("[%s %d]  error ret:%d!!\n",__FUNCTION__,__LINE__,ret);
		...
		/*选择AID*/
	{
		unsigned char recv_data[256];
		unsigned char recv_len[4];
		unsigned char send_len[] = {7,0,0,0}; 
		//unsigned char send_data[5] = {0x00,0xB0,0x96,0x00,0x06};
		unsigned char send_data[32] = {0x00,0xa4,0x00,0x00,0x02,0x3f,0x00};
		ret = PsamCmd(psamnum, send_data, send_len[0], recv_data, recv_len);	
		if(ret != 0)
		{
			printf("[%s %d]  error ret:%d!!\n",__FUNCTION__,__LINE__,ret);
	...
	/*选择文件*/
	{
		...
		unsigned char send_len[] = {5,0,0,0}; 
		//unsigned char send_data[5] = {0x00,0xB0,0x96,0x00,0x06};
		unsigned char send_data[32] = {0x00,0xb0,0x95,0x0,0x0};
		ret = PsamCmd(psamnum, send_data, send_len[0], recv_data, recv_len);	
		if(ret != 0)
		{
			printf("[%s %d]  error ret:%d!!\n",__FUNCTION__,__LINE__,ret);
	...
	/*选择文件*/
	{
		...
		unsigned char send_len[] = {5,0,0,0}; 
		//unsigned char send_data[5] = {0x00,0xB0,0x96,0x00,0x06};
		unsigned char send_data[32] = {0x00,0xb0,0x96,0x0,0x0};
		ret = PsamCmd(psamnum, send_data, send_len[0], recv_data, recv_len);	
		if(ret != 0)
		{
			printf("[%s %d]  error ret:%d!!\n",__FUNCTION__,__LINE__,ret);
