...
/***
  函 数 名 称:    DesCard
  功 能 描 述：    未找到其DES_CARD()函数的定义，可能未完成
  参 数 说 明：    KeyArray        :密钥串
                DataArray        :输入字符数组
                OutDataArray    :输出字符数组
...
/**************************************************************************************************************
- 函数说明 :初始化PSAM卡，并设置通信速率
- 输入参数 :
			PsamIndex:卡槽编号，范围3-5 
			BaudRate：通信速率，一般设置为9600或38400		
- 返回值:操作结果：0成功，其他失败 

原版无法使用疑似因为缺少某个初始化过程，补充后确认可以使用

**************************************************************************************************************/
JNIEXPORT jint JNICALL JNI_API_DEF(InitPsam)(JNIEnv *env, jobject obj,jint PsamIndex,jint BaudRate)                                                                
{
    (void)env;                                                                                                                               
    (void)obj;
////
...
- 输入参数 :
			PsamIndex:卡槽编号，范围3-5 
			sendcmd：发送的APDU命令，例如：{0x00,0xa4,0x00,0x00,0x02,0x3f,0x00}
			sendlen：发送的APDU命令长度；
			rcvcmd： 接收返回数据;
...
- 输入参数 :
			PsamIndex:卡槽编号，范围3-5 
			psamATR：PSAM卡复位返回值			
- 返回值:ATR数据长度，已被包含在InitPsam函数
**************************************************************************************************************/
JNIEXPORT jint JNICALL JNI_API_DEF(GetPsamAtr)(JNIEnv *env, jobject obj,jchar PsamIndex,jbyteArray psamATR)                                                                
{
    (void)env;                                                                                                                               
    (void)obj;	
    
    extern unsigned char PsamAtr[6][32];
    (*env)->SetByteArrayRegion(env, psamATR, 0, PsamAtr[PsamIndex][1], (jbyte *)&PsamAtr[PsamIndex][0]);
    
    
    LOGD("%s\n", __FUNCTION__);                                                                                                                                  

    return (jint)PsamAtr[PsamIndex][0];                                                                                                     
}
...
JNIEXPORT jint JNICALL JNI_API_DEF(GetPsamID)(JNIEnv *env, jobject obj,jint PsamIndex,jint BaudRate , jbyteArray psamID)                                                                
{
    ...
    InitPsam(PsamIndex,BaudRate);

    ret = GetPsamID_ex(PsamIndex,PsamNum,&PsamKeyIndex);
    if(ret != 0)
    {
